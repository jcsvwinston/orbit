...
`UI-06` and `UI-07` are **present** since A9 `S10`. The fleet's stubs are
connect-es 2 and protobuf-es 2, generated by `bufbuild/es` v2 alone from
`proto/` (messages and service descriptors from one generator; the code
creates messages with `create(Schema, …)` and calls through `createClient`).
