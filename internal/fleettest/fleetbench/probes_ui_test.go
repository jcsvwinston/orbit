...
// importedPaths lists the RELATIVE files a stylesheet or config imports,
// resolved to absolute paths. Package imports (no leading dot) are not a
// shared source inside this repository and are left out.
	...
	for _, re := range importRefs {
		for _, m := range re.FindAllStringSubmatch(string(raw), -1) {
			if strings.HasPrefix(m[1], ".") {
				out[filepath.Clean(filepath.Join(filepath.Dir(abs), m[1]))] = true
			}
		}
	}
	return out
...
// working-directory or in any of its steps.
func jobWorksIn(lines []string, dir string) bool {
	for _, l := range lines {
		if strings.TrimSpace(l) == "working-directory: "+dir {
			return true
		}
	}
	return false
...
var (
	specPathLiteral = regexp.MustCompile(`['"` + "`" + `](/[^'"` + "`" + `\s]*)['"` + "`" + `]`)
...
func probeFleetDistFreshnessGate(t *testing.T, e *env) verdict {
	...
	for name, lines := range jobs {
		if name == "admin-ui" || !jobWorksIn(lines, "ui") {
			continue
		}
		lanes++
	...
	t.Logf("%d job(s) work in ui/ and none diffs server/ui/dist after building", lanes)
	return absent
}
...
func probeFleetBundleBudget(t *testing.T, e *env) verdict {
	for _, f := range e.walkFiles(t, "server/ui", func(n string) bool { return strings.HasSuffix(n, "_test.go") }) {
		if _, _, ok := budgetConstants(e.readFile(t, f)); ok {
			t.Logf("%s names a budget", f)
			return present
		}
	}
	pkg := e.packageJSON(t, "ui/package.json")
...
// the runtime dependencies AND the generators that emit the stubs.
func probeConnectES2(t *testing.T, e *env) verdict {
	...
	depsV2 := connectMajor >= 2 && pbMajor >= 2

	generators := map[string]int{}
	for _, m := range bufGenPlugin.FindAllStringSubmatch(e.readFile(t, "proto/buf.gen.yaml"), -1) {
		v := -1
		if m[2] != "" {
	...
	genV2 := len(generators) == 2 && generators["bufbuild"] >= 2 && generators["connectrpc"] >= 2
	t.Logf("@connectrpc/connect %d, @bufbuild/protobuf %d; generators %v", connectMajor, pbMajor, generators)
	switch {
	case depsV2 && genV2:
		return present
	case depsV2 || genV2:
...
// UI-07: the browser instrument covers the fleet UI: a spec that visits a
// path outside the panel's /admin prefix.
func probeBrowserInstrumentCoversFleet(t *testing.T, e *env) verdict {
	var specs []string
	for _, root := range []string{"internal", "ui"} {
		specs = append(specs, e.walkFiles(t, root, func(n string) bool {
			return strings.HasSuffix(n, ".spec.ts") || strings.HasSuffix(n, ".spec.js") || strings.HasSuffix(n, ".spec.mts")
		})...)
	}
	if len(specs) == 0 {
		t.Log("no browser spec anywhere")
		return absent
	}
	var fleetVisits []string
	for _, f := range specs {
		for _, m := range specPathLiteral.FindAllStringSubmatch(e.readFile(t, f), -1) {
	...
	t.Logf("%d spec file(s), every path literal under /admin", len(specs))
	return absent
}
...
// is a test constant rather than a number in a document.
func probePanelBudgetEnforced(t *testing.T, e *env) verdict {
	js, css, ok := budgetConstants(e.readFile(t, "internal/admin/ui_embed_test.go"))
	if !ok {
